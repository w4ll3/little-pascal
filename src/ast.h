...
public:
  ...
  Statement(){};
  Statement(bool isFunction) : isFunction(isFunction){};
...
public:
  ...
  virtual std::vector<Node *> getChildren() {
    ...
    for (auto i : *(routine_part))
      list.push_back((Node *)i);
    for (auto i : *(routine_body->getlist()))
      list.push_back((Node *)i);
    return list;
  }
...
public:
  ...
  Routine(RoutineType rt, Identifier *rn, Parameters *vdl, Type *td)
      : Program(), routine_name(rn), return_type(td), argument_list(vdl),
        routine_type(rt) {}

  Routine(Routine *r, Program *p)
      : Program(*p), routine_name(r->routine_name), return_type(r->return_type),
        argument_list(r->argument_list), routine_type(r->routine_type) {}
  virtual ~Routine() {}
...
public:
  std::string name;
  bool isFuncAssig = false;

  Identifier(const std::string &name) : name(name) {}
  Identifier(const char *ptr_s) : name(*(new std::string(ptr_s))) {}
  Identifier(const std::string &name, bool isFuncAssig)
      : name(name), isFuncAssig(isFuncAssig) {}
...
public:
  ...
  AssignmentStmt(Identifier *lhs, Expression *rhs, bool isFunc)
      : lhs(lhs), rhs(rhs) {}
